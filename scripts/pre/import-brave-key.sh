...
rpmrebuild --change-spec-post="sed 's/service atd start//;s/echo \"sh \/etc\/cron.daily\/brave-key-updater\" | at now + 2 minute > \/dev\/null 2>&1//'" --define "BuildRoot $(pwd)/output" --define "_rpmdir $(pwd)" -p ./brave-keyring.rpm
